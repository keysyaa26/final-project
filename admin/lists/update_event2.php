...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // poster upload
        $foto = $event['poster']; // Menggunakan foto lama jika tidak ada foto baru yang diupload
        if (isset($_FILES['poster']) && $_FILES['poster']['error'] === UPLOAD_ERR_OK) {
            $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif'];
            $file_extension = strtolower(pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION));

            if (in_array($file_extension, $allowed_extensions)) {
            $file_name = 'poster_' . time() . '.' . $file_extension;
            $upload_dir = '../uploads/poster/';
            $upload_file = $upload_dir . $file_name;

            // Pindahkan file foto ke folder uploads
            if (move_uploaded_file($_FILES['poster']['tmp_name'], $upload_file)) {
                $foto = $file_name;
            } else {
        ...
        $stmt = $pdo->prepare("UPDATE events SET title = ?, event_type_ID = ?, venue_ID = ?, start_date =?, end_date =?, description =?, poster =?, price=?, status_acara =? WHERE event_ID = ?");
        if ($stmt->execute([$title, $event_type_id, $venue_id, $start_date, $end_date,  $description, $foto, $price, $status, $event_id])) {
        $updateSuccess = true; // Set flag to true if update is successful
            }

    
    // $acara = new Acara($title, $event_type_id, $venue_id, $start_date, $end_date,  $description, $status, $price);
